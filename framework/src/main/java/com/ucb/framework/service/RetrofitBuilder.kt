...
class RetrofitBuilder(val context: Context) {
    ...
    val dogService: IDogApiService = getRetrofit(BASE_URL_DOGS).create(IDogApiService::class.java)
    val marsService: IMarsApiService = getRetrofit(BASE_URL_MARS).create(IMarsApiService::class.java)

    companion object {
        private const val BASE_URL = "https://api.github.com"
        private const val BASE_URL_MOVIES = "https://api.themoviedb.org"
        private const val BASE_URL_DOGS = "https://dog.ceo"
        private const val BASE_URL_MARS = "https://api.nasa.gov/mars-photos/api/v1/"
    }
}
